"""Program 19: join_filter_groupby_sort — Join → Filter → GroupBy → Sort. Output ≈ 3×3."""

import fornero as pd
from tests.programs import ProgramResult
from tests.helpers.comparison import extract_source_data

PROGRAM_NAME = "join_filter_groupby_sort"
OPERATIONS = ["Source", "Join", "Filter", "GroupBy", "Sort"]
...
def run() -> ProgramResult:
    ...
    filtered = joined[joined["salary"] > 70000]
    grouped = filtered.groupby("dept").agg({"salary": "sum"})
    result = grouped.sort_values("salary", ascending=False)
